---
name: design
description: Define the user experience for a frontend feature. Writes the Design section to the Linear issue description. Run after /plan, before /refine.
...
Appends a `## Design` section to the Linear issue description with:
...
- Never overwrite sections owned by other commands (`## Plan`,
  `## Technical Spec`). Only touch `## Design`.
- `## Plan` is required. If the description lacks a Plan section,
...
- Re-running `/design` overwrites the existing Design section. The
  most recent design is the canonical design for downstream phases.
...
block for `## Plan > ### Problem Statement`,
`### Success Criteria`, and `### Acceptance Criteria`. If the Plan
section is missing, stop and tell the user to run `/plan` first.
...
### Step 2: Extract requirements from the Plan section

From the description's `## Plan` section, extract:
...
## Problem Context
{from the Linear issue description's ## Plan section — the problem and why we're solving it}
...
### Step 5: Merge the Design section into the description
...
- `## Design` (to be overwritten or inserted)
- `## Technical Spec` (preserved if exists)

**Build the new Design section:**
...
3. The new `## Design` section (from above)
...
description's `## Design` section.

## Integration with Workflow

```
/plan    -> WHY: Problem + success criteria (## Plan section)
          |
/design          -> WHAT: User experience + flows (## Design section)
          |
/refine    -> HOW: Technical implementation spec (## Technical Spec section)
          |
...
- Re-running `/design` overwrites the existing Design section
