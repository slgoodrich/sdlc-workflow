---
name: review
description: Fresh-context code review with parallel code, security, and performance agents. Prints verdict and auto-merges the PR on APPROVE.
argument-hint: <stream-id>
disable-model-invocation: true
allowed-tools: Bash(node *) Bash(git *) Bash(gh *) Bash(rm *)
---
...
conventions, and what the code SHOULD achieve (the plan + spec), but
NOT how or why it was built.
...
- Read requirements ONLY from the stream's `plan.md` and `spec.md`.
...
- **Acceptance criteria**: from plan.md `### Acceptance Criteria` (if
  present)
- **Technical Spec**: full spec.md content (required for a full review)

If spec.md is missing or empty, a code-quality-only review is still
possible, but flag it in the output.
...
{inlined acceptance criteria and file expectations from plan.md and spec.md, or 'No spec available - review code quality only' if empty}
...
(Code / Security / Performance) for the verdict output in Step 7.
...
2. Commit fixes: `git add -A && git commit -m "fix: address review findings for stream-$ARGUMENTS"`
3. Push: `git push`
...
### Step 7: Print combined review verdict

Print the merged findings from **all agents that ran** (code,
security, performance) to the terminal:
...
- Stream status (archived if merged, otherwise unchanged)
